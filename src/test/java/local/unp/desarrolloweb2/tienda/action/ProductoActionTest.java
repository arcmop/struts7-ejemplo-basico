...
public class ProductoActionTest {
    ...
    @Test
    public void testExecuteAppliesSearchByTituloAndKeepsNormalizedQuery() throws Exception {
        TiendaService tiendaService = mock(TiendaService.class);
        List<Producto> productos = Arrays.asList(
                new Producto(5, "Laptop HP 840 Gen3", "Core i5", 4000.0, 5)
        );
        when(tiendaService.getTotalProductosPorTitulo("laptop")).thenReturn(1);
        when(tiendaService.getListaProductosPaginadosPorTitulo("laptop", 1, 10)).thenReturn(productos);

        ProductoAction action = new ProductoAction();
        action.setTiendaService(tiendaService);
        action.setTituloBusqueda("  laptop  ");

        String result = action.execute();

        assertEquals("success", result);
        assertEquals("laptop", action.getTituloBusqueda());
        assertTrue(action.isBusquedaActiva());
        assertEquals(productos, action.getListaProducto());
        verify(tiendaService).getTotalProductosPorTitulo("laptop");
        verify(tiendaService).getListaProductosPaginadosPorTitulo("laptop", 1, 10);
        verify(tiendaService, never()).getTotalProductos();
    }

    @Test
    public void testExecuteIgnoresUnknownLoginErrorCode() throws Exception {
        TiendaService tiendaService = mock(TiendaService.class);
        when(tiendaService.getTotalProductos()).thenReturn(0);

        ProductoAction action = new ProductoAction();
        action.setTiendaService(tiendaService);
        action.setLoginErrorCode("title");

        String result = action.execute();

        assertEquals("success", result);
        assertEquals(null, action.getLoginError());
        verify(tiendaService).getTotalProductos();
    }
}
