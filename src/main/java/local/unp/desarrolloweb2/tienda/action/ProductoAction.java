...
public class ProductoAction extends ActionSupport implements SessionAware {
    ...
    private static final Set<String> LOGIN_ERROR_KEYS = createLoginErrorKeys();
    ...
    public boolean isBusquedaActiva() {
        return tituloBusqueda != null && !tituloBusqueda.isEmpty();
    }
    ...
    public String execute() throws Exception {
        this.tituloBusqueda = normalizeTexto(tituloBusqueda);
        int totalProductos = isBusquedaActiva()
                ? tiendaService.getTotalProductosPorTitulo(tituloBusqueda)
                : tiendaService.getTotalProductos();
        this.totalPaginas = totalProductos == 0 ? 1 : (int) Math.ceil((double) totalProductos / PRODUCTOS_POR_PAGINA);
        this.paginaActual = normalizePage(paginaSolicitada, totalPaginas);

        if (totalProductos == 0) {
            this.listaProducto = Collections.emptyList();
        } else {
            this.listaProducto = isBusquedaActiva()
                    ? tiendaService.getListaProductosPaginadosPorTitulo(tituloBusqueda, paginaActual, PRODUCTOS_POR_PAGINA)
                    : tiendaService.getListaProductosPaginados(paginaActual, PRODUCTOS_POR_PAGINA);
        }
        ...
        if (loginErrorCode != null && !loginErrorCode.trim().isEmpty()) {
            String key = loginErrorCode.trim();
            if (LOGIN_ERROR_KEYS.contains(key)) {
                this.loginError = getText(key);
            }
        }
    ...
    private static Set<String> createLoginErrorKeys() {
        Set<String> values = new HashSet<>();
        values.add("auth.error.invalid");
        values.add("auth.error.required");
        return values;
    }
}
